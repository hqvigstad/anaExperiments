...
/// Usage:
/// aliroot -b -q -l setupSingleTask.C'("localAodDirectory", nFiles)' run-single-task.C'(...)'
/// aliroot -b -q -l setupSingleTask.C'("lhcPeriod", "mcPeriod")' run-single-task.C'(...)'
///
/// Example:
/// aliroot -b -q -l setupSingleTask.C run-single-task.C'(...)'
///
...
void setupSingleTask(const char* lhcPeriod=NULL, const char* mcProd="")
{
  // Grid mode with optional calling of AddGoodRuns for specified
  // period
  setupSingleTask(NULL, 0, lhcPeriod, mcProd);
}

void setupSingleTask(const char* localAodDirectory, int nFiles)
{
  // local mode for AOD data
  setupSingleTask(localAodDirectory, nFiles, NULL);
}
